...
//
//	modified by GatorV 7 July 2006 - Fixed popup position and DOCTYPE position, DOM calendar appended to the bot of the calendar
//
...
//	email  : pinotoy@yahoo.com
	...
	var imgDir = '/library/Gecko/Assets/JavaScriptHelpers/popcal2/';		// directory for images ... e.g. var imgDir="/img/"
	var dayName = '';
	...
	var     monthName2 = {
			...
			ru : new Array('§Á§¯§£','§¶§Š§£','§®§¡§²','§¡§±§²','§®§¡§«','§ª§À§¯','§ª§À§­','§¡§£§€','§³§Š§¯','§°§¬§Ž','§¯§°§Á','§¥§Š§¬')
	};
	...
	*/
	...
	function init() {
		...
		var theHTML = '';
		theCalendar.id = "calendar";
		theCalendar.onclick = function() { bShow = true; };
		theCalendar.style.cssText = "z-index:+999;position:absolute;visibility:hidden;";
		theHTML = '<table width="'+((showWeekNumber==1)?250:220)+'" style="font-family:Arial;font-size:11px;border: 1px solid #A0A0A0;" bgcolor="#ffffff"><tr bgcolor="#000066"><td><table width="'+((showWeekNumber==1)?248:218)+'"><tr><td style="padding:2px;font-family:Arial;font-size:11px;"><font color="#ffffff' + '' /*C9D3E9*/ +'"><b><span id="caption"></span></b></font></td><td align="right"><a href="javascript:hideCalendar()"><img src="'+imgDir+'close.gif" width="15" height="13" border="0" /></a></td></tr></table></td></tr><tr><td style="padding:5px" bgcolor="#ffffff"><span id="InnerCalendarContent"></span></td></tr>';

		if (showToday == 1) {
			theHTML += '<tr bgcolor="#f0f0f0"><td style="padding:5px" align="center"><span id="lblToday"></span></td></tr>';
		}
		
		theHTML += '</table><div id="selectMonth" style="z-index:+999;position:absolute;visibility:hidden;"></div><div id="selectYear" style="z-index:+999;position:absolute;visibility:hidden;"></div>';
		
		theCalendar.innerHTML = theHTML;
	...
	function popUpMonth() {
		if (visMonth == 1) {
			...
			*/
			crossMonthObj.left = "50px";
			crossMonthObj.top =	"26px";
	...
	function popUpYear() {
		...
		if (visYear==1) {
			...
			*/
			crossYearObj.left = "96px";
			crossYearObj.top = "26px";
			visYear++;
		}
	}
	...
	function constructCalendar () {
		...
		document.getElementById('InnerCalendarContent').innerHTML   = sHTML
	...
	function showCalendar(ctl, ctl2, format, lang, past, fx, fy, func) {
	...
	function popUpCalendar(ctl, ctl2, format) {
		...
		if (bPageLoaded) {
			if (crossobj.visibility == 'hidden') {
				...
				/** Get Parent Pos Fixed for FF && IE && Opera **/
				/*
				do {
				...
				*/
				leftpos = findPosX( aTag.offsetParent );
				toppos = findPosY( aTag.offsetParent );
